...
func main() {
	...
	if serviceCfg.KubeconfigKeychainConfig.EnableKeychain {
		...
		if kcp := serviceCfg.KubeconfigKeychainConfig.KubeconfigPath; kcp != "" {
	...
	httpConfig := serviceCfg.RetryableHTTPClientConfig
	registryConfig := serviceCfg.ResolverConfig

	hosts := resolver.NewRegistryManager(httpConfig, registryConfig, credsFuncs).AsRegistryHosts()
	...
	if !serviceCfg.DisableVerification {
		log.G(ctx).Warnf("content verification is not supported; switching to non-verification mode")
		serviceCfg.DisableVerification = true
	}
	mt, err := getMetadataStore(*rootDir, serviceCfg)
	if err != nil {
	...
	fs, bgFetcher, err := socifs.NewFilesystem(ctx, defaultRootDir, serviceCfg.FSConfig, fsOpts...)
	if err != nil {
		log.G(ctx).WithError(err).Fatalf("failed to prepare fs")
	}

	layerManager, err := store.NewLayerManager(ctx, *rootDir, (source.RegistryHosts)(hosts), mt, fs, bgFetcher, serviceCfg.FSConfig)
	if err != nil {
...
func getMetadataStore(rootDir string, serviceCfg config.ServiceConfig) (metadata.Store, error) {
	bOpts := bolt.Options{
		NoFreelistSync:  true,
