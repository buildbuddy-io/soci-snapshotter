...
type fs struct {
	...
	knownNode   map[string]map[string]*layerReleaseable
	knownNodeMu sync.Mutex
}

type layerReleaseable struct {
	n        fusefs.InodeEmbedder
	released bool
	mu       sync.Mutex
}

func (lh *layerReleaseable) releaseable() bool {
	lh.mu.Lock()
...
func (lh *layerReleaseable) release() {
	lh.mu.Lock()
	lh.released = true
	lh.mu.Unlock()
}
...
type inoReleaseable struct {
	n fusefs.InodeEmbedder
}

func (r *inoReleaseable) releaseable() bool {
	return r.n.EmbeddedInode() != nil && r.n.EmbeddedInode().Forgotten()
}
...
func (n *layernode) Lookup(ctx context.Context, name string, out *fuse.EntryOut) (*fusefs.Inode, syscall.Errno) {
	...
	case layerLink, blobLink:
		...
		err = n.fs.layerMap.add(ctx, func(id uint32) (releaseable, error) {
			...
			if errno != 0 {
				return nil, fmt.Errorf("failed to get root node 1: %v", errno)
			}
			...
			rr := &layerReleaseable{n: root}
			n.fs.knownNodeMu.Lock()
			if n.fs.knownNode == nil {
				n.fs.knownNode = make(map[string]map[string]*layerReleaseable)
			}
			if n.fs.knownNode[n.refnode.ref.String()] == nil {
				n.fs.knownNode[n.refnode.ref.String()] = make(map[string]*layerReleaseable)
			}
...
// idMap manages uint32 IDs with automatic GC for releaseable objects.
type idMap struct {
	m        map[uint32]releaseable
	max      uint32
	mu       sync.Mutex
...
func (m *idMap) add(ctx context.Context, p func(uint32) (releaseable, error)) error {
	...
	if m.m == nil {
		m.m = make(map[uint32]releaseable)
	}
